...
def generate_pert_samples_with_labels(dataset, conf, max_num, itemset, sess, x, preds, is_satisfy, is_positive):
    ...
    def change_sensitive_range(x, itemset):
        for item in itemset:
            ...
            x[feature_index] = round(random.uniform(feature_range[0], feature_range[1]), 2)
        return x
...
# 0.7, 0.5~1
all_itemsets = [['recePctWhite=(0,0.4)', 'FemalePctDiv=(0.7,1.0)']]

for itemset in all_itemsets:
    print("-->itemset:", itemset)
    # all_data = generate_random_data_itemset(1000, conf, sess, x, preds, itemset, True)
    generated_data = generate_pert_samples_with_labels(list(X), conf, 1000, itemset, sess, x, preds, True, False)

print("-->generated_data")
print(generated_data)
file_path = "../dataset/communities_2.csv"
data_pd = DataFrame(generated_data)
data_pd.to_csv(file_path)

generated_labels = []
for i in range(0, len(generated_data)):
    generated_labels.append([1.0, 0.0])
