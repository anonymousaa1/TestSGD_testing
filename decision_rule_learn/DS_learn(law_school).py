...
dataset = "law_school"
filename = "../dataset/law_school.csv"
...
# saver.restore(sess, "../models/law_school/999/test.model")
saver.restore(sess, "../models/law_school_1/999/test.model")

# retrained model path8
# saver.restore(sess, "../models/law_school15/999/test.model")
...
print(all_itemsets)
print("-->all_itemsets", len(all_itemsets))
...
# all_itemsets = [['sex=1', 'race=3']]
# all_itemsets = [['sex=1', 'race=(2, 3)']]
# all_itemsets = [['sex=1', 'race=(2, 3)']]
import time
start= time.time()
...
for itemset in all_itemsets:
    ...
    # all_data = generate_random_data_itemset(1000, conf, sess, x, preds, itemset, True)
    all_data = generate_pert_samples(list(X), conf, 1000, itemset, sess, x, preds, True) #300
    labels = list(np.array(all_data)[:, -1])
    p1, purt1 = calculate_confidence_interval(labels)
...
print("-->max scores for p1 and p2", all_itemsets[all_fair_scores.index(max(all_fair_scores))])
