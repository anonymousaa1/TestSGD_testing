...
def generate_all_itemset_range(conf):
    ...
    race_feature = conf.sensitive_feature_name[1:4]
    race_index = conf.sensitive_params[1:4]
    sex_feature = conf.sensitive_feature_name[8:11]
    sex_index = conf.sensitive_params[8:11]

    # print("-->race/sex_index", race_index, sex_index)

    race_com = list(itertools.product(race_index, com))
    print("race_com", len(race_com))
...
saver.restore(sess, "../models/communities(original)/999/test.model")
# retrained model path
# saver.restore(sess, "../models/communities1000/999/test.model")
...
# all_itemsets = [['recePctWhite=(0,0.7)'], ['agePct16t24=(0.3,0.8)'], ['FemalePctDiv=(0,0.4)'], ['recePctWhite=(0,0.7)', 'FemalePctDiv=(0.5,1)'],
#                 ['recePctWhite=(0,0.7)', 'FemalePctDiv=(0.2,0.7)'], ['recePctWhite=(0,0.8)', 'FemalePctDiv=(0.3,0.8)'],
#                 ['recePctWhite=(0,0.6)', 'MalePctNevMarr=(0.6,0.9)'], ['recePctWhite=(0,0.7)', 'MalePctNevMarr=(0.8,0.9)']]

# all_itemsets = [ ['recePctWhite=(0,0.7)', 'FemalePctDiv=(0.5,1.0)']]
import time
start = time.time()
